...
#### 📌블록요소들은 새로운 줄에서 시작, 인라인 요소는 텍스트 주변 사이에 위치



### 요소의 위치제어

**정상적인 흐름**: 모든 블록레벨 요소는 새로운 줄에 위치하므로 페이지가 길어짐

> `position-static`: 기본 방식이므로 따로 적지 않아도 적용됨

**상대 위치**: 정상적인 흐름의 위치를 기준으로 상하좌우 상대적인 위치를 나타냄

> `position: relative`
>
> ``` css
> /*예시:p태그 안에 3가지 문단 one two three가 있는 상황*/
> .two{
>     position: relative;
>     top: 10px;
>     left: 100px;
...
**절대 위치**: 부모 요소를 기준으로 요소를 배치해 정상적인 흐름을 무시하고 주어진 위치에 고정됨. 스크롤을 하여도 부모요소 기준으로 잡힌 절대위치를 벗어나지 않음. 다른 요소에 영향을 주지 않음. 원래 그 위치에 없는 듯이 동작함

> `position: absoulte`
>
> ``` css
> /*예시: 상단에 붙고 왼쪽에서 500px 떨어져 있는 너비 250px짜리 h1 요소*/
> h1{
>     position: absolute;
>     background-color: #efefef;
>     top: 0px;
...
**고정 위치**: 부모 요소가 아닌 브라우저를 기준으로 요소를 고정함. 어떠한 요소에 영향을 받지않고 스크롤 하여도 기준 위치를 벗어나지 않음. 보통 브라우저의 상단으로 이동시켜주는 top 버튼등이 고정위치 요소이다

> `position-fixed`
>
> ``` css
> /*예시: 우측 하단에 있는 top버튼 요소*/
> .top_btn{
>     position: fixed;
>     top: auto;
>     bottom: 50px;
...
**부유 요소**: 정상적인 흐름에서 벗어나 포함하는 박스의 왼쪽이나 오른쪽에 떨어져 위치한다. 부유 요소 주위에 다른 콘텐츠가 위치할 수 있는 블록요소가 된다.
...
<body>
  ...
  <p>
    <h5 class="one">첫번째 문단</h5>
    <br>
    <h5 class="two">두번째 문단</h5>
    <br>
    <h5 class="three">세번째 문단</h5>
  </p>
  <div id="bycycle">
    <p>자전거</p>
    <blockquote>
      Life is like riding a bycycle bucause...
    </blockquote>
  </div>
